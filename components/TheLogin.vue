<template>
  <div v-if="hide" class="login">
    <div class="login-modal">
      <div @click="$store.dispatch('loginModal', false)" class="w-10 h-10 rounded-lg bg-gray-100 flex items-center justify-center relative x-position cursor-pointer">
        <the-icon src="x" />
      </div>
      ...
      <p class="text-lg text-center text-gray-700 mt-1">У вас ещё нет аккаунта?
        <span @click="toRegister" class="text-orange-600 cursor-pointer font-semibold	">Регистрация</span>
      </p>
      <input
